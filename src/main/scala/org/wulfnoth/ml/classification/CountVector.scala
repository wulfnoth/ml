...
case class CountVector(dimSize: Int) {
	...
	//所有属性（纬度）的各个值的计数
	private[ml] val attributes = new SparseArray[AttributeValues[Int]](dimSize)

	def length: Long = size

	def update(dim: Int, value: Int): Unit = {
		val attribute = attributes.getOrElseUpdate(dim, new AttributeValues[Int])
